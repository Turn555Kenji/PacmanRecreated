...
func set_current_intersection(inter: Intersection): #called by intersection signal
	...
	if run_away_state:
		...
		var pinky_target = get_pinky_target_position(pacman)
		direction = get_astar_direction_to_position(pinky_target)
		
func get_pinky_target_position(pacman: CharacterBody2D, tile_size: float = 16.0) -> Vector2:
	var look_ahead_tiles = 4
	var offset = pacman.velocity.normalized() * tile_size * look_ahead_tiles
	return pacman.global_position + offset
...
func get_astar_direction_to_position(target_pos: Vector2) -> Vector2:
	...
	#debug.set_debug_path(reconstruct_path(came_from, current_intersection, target_intersection), Color.PINK)
...
func run_away():
	if house_timer >= 0.0:
		return
	print(run_away_state, run_away_timer)
	if run_away_state and run_away_timer >= 0:
		run_away_timer = 7.0
		return
	run_away_state = true
...
func run_away_count_down(delta):
	...
	if run_away_timer <= 0:
		...
		var texture = load("res://Assets/sprites/pinky.png")
		$Sprite2D.texture = texture
		run_away_timer = 7.0
		speed = 70.0
